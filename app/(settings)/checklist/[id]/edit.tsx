...
export default function EditChecklistScreen() {
  ...
  const { items, updateItem, isLoading } = useChecklist();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [showConfirm, setShowConfirm] = useState(false);
  const [pendingUpdate, setPendingUpdate] = useState<Checklist | null>(null);
  
  const item = items.find(i => i.id === parseInt(id));
  
  const [form, setForm] = useState({
    symptom: item?.symptom ?? '',
    risk_weight: item?.risk_weight.toString() ?? '',
  ...
  const validateSymptom = async (symptom: string) => {
    ...
    // Check for duplicate symptoms (case-insensitive), excluding the current item
    const normalizedSymptom = symptom.trim().toLowerCase();
    const isDuplicate = items.some(i => 
      i.id !== parseInt(id) && 
      i.symptom.toLowerCase() === normalizedSymptom
    );
    if (isDuplicate) {
  ...
  const handleSubmitConfirm = async () => {
    ...
    try {
      setIsSubmitting(true);
      await updateItem(pendingUpdate);
      router.back();
    } catch (e) {
    ...
    <>
          ...
          <ThemedView style={styles.buttonContainer}>
            ...
            >
              {isSubmitting ? (
                ...
                <ThemedText style={styles.submitButtonText}>Update Checklist Item</ThemedText>
              )}
            </ThemedView>
          </ThemedView>
        </ThemedView>
      ...
      {showConfirm && (
        <ConfirmDialog
          ...
          message="Are you sure you want to update this checklist item?"
